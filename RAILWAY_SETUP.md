# RelicForge V11.1.3 — Railway + Alchemy Setup
...
- A Railway account.
- An Alchemy account. V11.1.3 uses one private API key with a built-in EVM endpoint registry instead of one Railway variable per chain.
...
V11.1.3 no longer stores a full Alchemy URL per network. Add **one private variable** to the Railway API service:
...
# Studio cloud project quota
PROJECT_LIMIT=10
# Mint-page collection images and banners are hard-limited by the API to 2 MB each.
