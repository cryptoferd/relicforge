...
## V11.1.3 — wallet switching + save sign-in gate
...
## V11.1.3 — readable per-trait rarity tiers
...
## V11.1.3 — View Image data-URI compatibility fix
...
## V11.1.3 — Studio UX + standalone Creator Dashboard
...
## V11.1.3 — public gas oracle + full collection preview
...
# Relic Forge Studio V11.1.3 — Cloud + Alchemy + Render Modes


## Animated GIF artwork (V11.1.3)
...
The Alchemy key stays **only on Railway**. V11.1.3 requires a single `ALCHEMY_API_KEY`; endpoint bases are non-secret application configuration in `server/src/lib/alchemy-networks.js`. Do not put the key in `relicforge-config.js`, GitHub Pages, Vercel client variables, or a standalone mint page.
