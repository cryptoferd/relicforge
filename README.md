...
## V11.1.2 — readable per-trait rarity tiers
...
## V11.1.2 — View Image data-URI compatibility fix
...
## V11.1.2 — Studio UX + standalone Creator Dashboard
...
## V11.1.2 — public gas oracle + full collection preview
...
# Relic Forge Studio V11.1.2 — Cloud + Alchemy + Render Modes


## Animated GIF artwork (V11.1.2)
...
The Alchemy key stays **only on Railway**. V11.1.2 requires a single `ALCHEMY_API_KEY`; endpoint bases are non-secret application configuration in `server/src/lib/alchemy-networks.js`. Do not put the key in `relicforge-config.js`, GitHub Pages, Vercel client variables, or a standalone mint page.
