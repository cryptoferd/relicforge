(() => {
  ...
  const V2_FACTORY_ABI = [
    ...
    'function mintPhasesForCollection(address collection) view returns(address)',
    'function dataForCollection(address collection) view returns(address)',
    'function isRelicForgeCollection(address collection) view returns(bool)',
    'function collectionImplementation() view returns(address)',
    'function dataImplementation() view returns(address)',
    'function mintPhasesImplementation() view returns(address)',
    'function renderer() view returns(address)',
    'function randomnessProvider() view returns(address)',
    ...
    'event CollectionLaunchTerms(address indexed collection,uint8 feeMode,uint32 lockedFeeCents,uint256 sponsoredPrepaidWei,uint8 initialRevealMode,uint64 batchWindowSeconds,uint256 maxRandomnessCostPerBatchWei)'
  ];

  const V2_COLLECTION_ABI = [
    'function name() view returns(string)', 'function symbol() view returns(string)', 'function description() view returns(string)',
    'function creator() view returns(address)', 'function owner() view returns(address)', 'function controller() view returns(address)',
    ...
    'function batchWindowSeconds() view returns(uint64)', 'function maxRandomnessCostPerBatchWei() view returns(uint256)', 'function autoRevealCallbackGasForQuantity(uint32) view returns(uint32)',
  ...
  const V2_MINT_PHASES_ABI = [
    'function collection() view returns(address)', 'function controller() view returns(address)', 'function masterMintEnabled() view returns(bool)',
    'function phaseCount() view returns(uint32)',
    'function phases(uint32) view returns(uint96 price,uint64 startTime,uint64 endTime,uint32 phaseSupply,uint32 minted,uint32 maxPerWallet,bytes32 merkleRoot,uint8 accessType,uint16 priority,bool enabled)',
    'function phaseWalletMinted(uint32,address) view returns(uint32)',
    'function phaseIsOpen(uint32 phaseId) view returns(bool)',
    'function quoteMint(uint32 phaseId,uint32 quantity) view returns(uint256 creatorPrice,uint256 platformFeeWei,uint256 minimumValue,bool oracleHealthy,bool feeActive)',
    'function creatorTeamFeeQuote(uint32 quantity) view returns(uint256 feeWei,bool oracleHealthy,bool feeActive)',
    'function setMasterMintEnabled(bool enabled)',
    'function createPhase(uint96 price,uint64 startTime,uint64 endTime,uint32 phaseSupply,uint32 maxPerWallet,bytes32 merkleRoot,uint8 accessType,uint16 priority,bool enabled) returns(uint32 phaseId)',
    'function updatePhase(uint32 phaseId,uint96 price,uint64 startTime,uint64 endTime,uint32 phaseSupply,uint32 maxPerWallet,bytes32 merkleRoot,uint8 accessType,uint16 priority)',
    'function setPhaseEnabled(uint32 phaseId,bool enabled)',
  ...
  async function isCanonicalV2Collection(collectionAddress) {
    ...
    try {
      ...
      return String(factory).toLowerCase() === String(cfg.factory).toLowerCase();
  ...
  async function buildMintPageConfig(collectionAddress = forgeState.collectionAddress) {
    ...
    const isV2MintPage = await isCanonicalV2Collection(collectionAddress);
    ...
    return {
      schema: isV2MintPage ? 'relic-forge/mint-page@3' : 'relic-forge/mint-page@1',
      protocol: isV2MintPage ? 'r12-v2' : 'legacy',
  ...
  async function publishMintPageCloud(collectionAddress = forgeState.collectionAddress, dashboard = false) {
    const isV2MintPage = await isCanonicalV2Collection(collectionAddress);
    const mintPageChainId = activeChainId() || 11155111;
    ...
    const localConfig = dashboard
      ? {
          ...readMintPageConfig(collectionAddress, mintPageChainId),
          schema: isV2MintPage ? 'relic-forge/mint-page@3' : 'relic-forge/mint-page@1',
          protocol: isV2MintPage ? 'r12-v2' : 'legacy',
          chainId: mintPageChainId,
          contract: collectionAddress,
          generatedAt: new Date().toISOString(),
        }
...
${await file.text()}`;
  ...
  function updateRevealUi() {
    ...
    $('creatorPlaceholderWrap')?.classList.remove('hidden');
    bridge().updateLaunchSummary?.();
  ...
  function renderCompileReport() {
    ...
    $('forgeValidationList').innerHTML = [
      ...
      `<div class="forge-check good">✓ ${c.core.revealMode === 0 ? 'Deferred Reveal' : 'Forge Reveal'} configured</div>`,
  ...
  function legacyV1Config() {
    const config = window.RELICFORGE_V1_ADDRESSES?.[11155111];
    return config && window.ethers?.isAddress(config.factory) ? config : null;
  }
  ...
  function renderCanonicalV1() {
    try {
      ...
      const fields = {
        ...
        canonicalReserveAddress: cfg.reserve,
        canonicalMintPhasesAddress: cfg.mintPhasesImplementation
      };
  ...
  async function refreshPlatformFeeQuote() {
    ...
    try {
      ...
      const name = mode === V1_FEE_MODE_SPONSORED ? 'Creator Covers Platform Fee' : 'Collector Covers Platform Fee';
      if ($('platformFeePolicyLabel')) $('platformFeePolicyLabel').textContent = name + (active ? ' - active' : ' - $0 policy');
      if ($('platformFeeRate')) $('platformFeeRate').textContent = mode === V1_FEE_MODE_SPONSORED
        ? '$' + (rate/100).toFixed(2) + ' x ' + supply.toLocaleString() + ' max supply'
        : '$' + (rate/100).toFixed(2) + ' / NFT';
      if ($('platformFeeUpfront')) $('platformFeeUpfront').textContent = mode === V1_FEE_MODE_SPONSORED
        ? Number(window.ethers.formatEther(upfront)).toFixed(6) + ' ETH'
        : '0 ETH';
      if ($('platformFeeQuoteStatus')) $('platformFeeQuoteStatus').textContent = healthy
        ? 'Live R12-v2 fee quote. Fee value enters the collection reveal-funding path.'
        : (mode === V1_FEE_MODE_SPONSORED
          ? 'Creator Covers Platform Fee launch is unavailable until the ETH/USD oracle is healthy.'
          : 'Oracle unavailable; the current Collector Covers Platform Fee mint path fails open to $0 platform fee.');
    } catch (error) {
  ...
  function collectionContract(runner = forgeState.signer) {
    if (!forgeState.collectionAddress || !window.ethers.isAddress(forgeState.collectionAddress)) throw new Error('No forged R12-v2 collection is loaded.');
    if (!runner) throw new Error('Connect the creator wallet first.');
    return new window.ethers.Contract(forgeState.collectionAddress, V2_COLLECTION_ABI, runner);
  }

  function mintPhasesContract(runner = forgeState.signer) {
    if (!forgeState.mintPhasesAddress || !window.ethers.isAddress(forgeState.mintPhasesAddress)) throw new Error('No R12-v2 MintPhases clone is loaded.');
    if (!runner) throw new Error('Connect the creator wallet first.');
    return new window.ethers.Contract(forgeState.mintPhasesAddress, V2_MINT_PHASES_ABI, runner);
  }
  ...
  async function armMasterMint() {
    ...
    await requireForgeWrite(true);
      ...
      if ($('forgeArmMintBtn')) { $('forgeArmMintBtn').disabled = true; $('forgeArmMintBtn').textContent = 'Minting Enabled'; }
      ...
      log('forgeTestStatus','Minting enabled. Individual stage schedules and limits still apply.');
  ...
  async function loadViewerCollection(resetPage = true) {
    try {
      ...
      setViewerMetric('viewerMetricReveal', Number(revealMode) === 0 ? 'Deferred Reveal' : 'Forge Reveal');
  ...
  function knownFactories() {
    ...
    try { push(legacyV1Config()?.factory); } catch (_) {}
  ...
  async function rf26V2CreatorMintOverrides(collection, quantity, value) {
    const revealMode=Number(await collection.futureRevealMode());
    if(revealMode!==1)return { value };

    const gasPrice=await rf26RandomnessGasPrice();
    const maxGroupQuantity=Math.min(20,Math.max(1,Number(quantity)));
    const [callbackGasRaw,ceilingRaw,adapterAddress]=await Promise.all([
      collection.autoRevealCallbackGasForQuantity(maxGroupQuantity),
      collection.maxRandomnessCostPerBatchWei(),
      collection.randomnessProvider(),
    ]);
    const callbackGas=Number(callbackGasRaw);
    const ceiling=BigInt(ceilingRaw);
    const adapter=new window.ethers.Contract(adapterAddress,V2_RANDOMNESS_ABI,readProvider(activeChainId() || 11155111));
    const quote=BigInt(await adapter.estimateRequestPriceAtGasPrice(callbackGas,gasPrice));
    if(quote>ceiling)throw new Error(
      'Creator Mint temporarily unavailable: current randomness quote '+window.ethers.formatEther(quote)+
      ' ETH exceeds the platform ceiling '+window.ethers.formatEther(ceiling)+' ETH.'
    );

    const estimate=BigInt(await collection.creatorMint.estimateGas(
      forgeState.wallet,quantity,{value,gasPrice}
    ));
    const gasLimit=((estimate*130n)/100n)+100000n;
    console.info('RelicForge R2 fee-aware Creator Mint',{
      quantity:Number(quantity),callbackGas,gasPriceWei:gasPrice.toString(),
      randomnessQuoteWei:quote.toString(),randomnessCeilingWei:ceiling.toString(),
      gasEstimate:estimate.toString(),gasLimit:gasLimit.toString()
    });
  ...
  async function handleV2LaunchedAction(action, snap) {
    try {
      ...
      const canControl = String(snap.controller).toLowerCase() === String(forgeState.wallet).toLowerCase();
      if (['mastermint','creatormint','savemintpage','preparedeferred','requestdeferred','withdraw','payout','royalty'].includes(action) && !canControl) {
      ...
      if (action === 'mastermint') {
        ...
        const remainingSupply=Math.max(0,Number(snap.maxSupply)-Number(snap.totalMinted));
        const q = Math.max(1, Math.min(50, remainingSupply, Math.floor(Number($('dashboardV2CreatorMintQty')?.value || 1))));
        const quote = await phases.creatorTeamFeeQuote(q);
        const fee = BigInt(quote.feeWei ?? quote[0] ?? 0n);
        const healthy = Boolean(quote.oracleHealthy ?? quote[1]);
        const active = Boolean(quote.feeActive ?? quote[2]);
        const value = active && healthy ? fee : 0n;
        launchedStatus('Creator mint fee quote: ' + window.ethers.formatEther(value) + ' ETH. Preparing adaptive Forge gas envelope...');
        const overrides=await rf26V2CreatorMintOverrides(collection,q,value);
        const tx = await collection.creatorMint(forgeState.wallet, q, overrides);
        await tx.wait();
        launchedStatus('Creator mint confirmed. NFT ownership is immediate; Forge reveal continues automatically when configured.');
      } else if (action === 'savemintpage') {
        const chainId=activeChainId() || 11155111;
        const existing={...readMintPageConfig(snap.address,chainId),...(await publishedMintPageConfig(snap.address,chainId))};
        const config={
          ...existing,
          ...
          chainId,
          contract:snap.address,
          title:String($('dashboardV2MintPageTitle')?.value || snap.name || 'Relic Forge Collection').trim(),
          description:String($('dashboardV2MintPageDescription')?.value || snap.description || '').trim(),
          updatedAt:new Date().toISOString(),
        };
        delete config.collectionImage;
        delete config.bannerImage;
        writeMintPageConfig(config);
        launchedStatus('Publishing R12-v2 mint-page settings and media…');
        await publishMintPageCloud(snap.address,true);
        forgeState.dashboardMintPageImageFile=null;
        forgeState.dashboardMintPageBannerFile=null;
        launchedStatus('Mint page updated. Replaced media is removed from RelicForge storage when no longer referenced.');
  ...
  async function openV2LaunchedCollection(snap) {
    ...
    const v2MintPageBanner=v2MintPageConfig.bannerImage || '';
    const savedProjectMatches=String(forgeState.deploymentJournal?.collectionAddress || '').toLowerCase()===String(snap.address).toLowerCase();
    if(savedProjectMatches && !v2MintPageImage && forgeState.mintPageImageFile)forgeState.dashboardMintPageImageFile=forgeState.mintPageImageFile;
    if(savedProjectMatches && !v2MintPageBanner && forgeState.mintPageBannerFile)forgeState.dashboardMintPageBannerFile=forgeState.mintPageBannerFile;
    ...
    detail.innerHTML =
      ...
      '<div class="launched-section" id="r23StageQuickAnchor"><h4>MintPhases stages</h4>' + v2PhaseRows(snap, canControl) + '<div class="r23-mobile-quick-actions"><button class="primary-btn" data-r23-quick-create="1" type="button" ' + (canControl ? '' : 'disabled') + '>+ Add Allowlist Phase</button><small>Create a new Approved Wallet stage after launch. Stage configuration remains available even if the collection is already sold out.</small><div class="r23-mobile-quick-status" id="r23MobileQuickStatus" data-tone="neutral">' + (canControl ? 'Ready to open the Approved Wallet stage creator.' : 'Connect the active controller wallet to create a stage.') + '</div></div><div class="prototype-note"><strong>Stage scheduling remains independent of reveal</strong><p>Existing stages can be updated, enabled, or disabled here. A sold-out collection stays closed even if a stage schedule is otherwise open.</p></div></div>' +
    ...
    $('dashboardV2MintPageImageInput')?.addEventListener('change', async event => {
      try {
        forgeState.dashboardMintPageImageFile=validateMintPageMedia(event.target.files?.[0] || null,'Collection image');
        if($('dashboardV2MintPageImageName'))$('dashboardV2MintPageImageName').textContent=forgeState.dashboardMintPageImageFile
          ? forgeState.dashboardMintPageImageFile.name+' · '+(forgeState.dashboardMintPageImageFile.size/1024/1024).toFixed(2)+' MB'
          : (v2MintPageImage?'Current image saved · choose a file to replace it':'2 MB max · image file');
        const preview=forgeState.dashboardMintPageImageFile?await fileToDataUrl(forgeState.dashboardMintPageImageFile):v2MintPageImage;
        setPreviewImage('dashboardV2MintPagePreviewImage',preview,'RF');
      } catch(error) {
        event.target.value='';
        forgeState.dashboardMintPageImageFile=null;
        if($('dashboardV2MintPageImageName'))$('dashboardV2MintPageImageName').textContent='Image rejected: '+error.message;
      }
    });
    $('dashboardV2MintPageBannerInput')?.addEventListener('change', async event => {
      try {
        forgeState.dashboardMintPageBannerFile=validateMintPageMedia(event.target.files?.[0] || null,'Collection banner');
        if($('dashboardV2MintPageBannerName'))$('dashboardV2MintPageBannerName').textContent=forgeState.dashboardMintPageBannerFile
          ? forgeState.dashboardMintPageBannerFile.name+' · '+(forgeState.dashboardMintPageBannerFile.size/1024/1024).toFixed(2)+' MB'
          : (v2MintPageBanner?'Current banner saved · choose a file to replace it':'2 MB max · image file');
        const preview=forgeState.dashboardMintPageBannerFile?await fileToDataUrl(forgeState.dashboardMintPageBannerFile):v2MintPageBanner;
        setPreviewImage('dashboardV2MintPagePreviewBanner',preview,'BANNER');
      } catch(error) {
        event.target.value='';
        forgeState.dashboardMintPageBannerFile=null;
        if($('dashboardV2MintPageBannerName'))$('dashboardV2MintPageBannerName').textContent='Banner rejected: '+error.message;
      }
    });
  ...
  function restoreForgeProjectState(saved, options = {}) {
    ...
    if (saved.collectionAddress && window.ethers?.isAddress(saved.collectionAddress)) {
      ...
      forgeState.mintPhasesAddress = saved.mintPhasesAddress && window.ethers?.isAddress(saved.mintPhasesAddress) ? saved.mintPhasesAddress : null;
  ...
  function getCompiledSummary() {
    ...
    return {
      ...
      revealMode: c.core.revealMode === 0 ? 'deferred' : 'forge',
    };
  }
  ...
  function bind() {
    ...
    if ($('publishMintPageBtn')) $('publishMintPageBtn').disabled = true;
    if ($('openMintPageBtn')) $('openMintPageBtn').disabled = true;
