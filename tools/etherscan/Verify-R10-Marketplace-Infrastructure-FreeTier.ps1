...
function Fail-R10Verify([string]$Message) {
    throw "R10 MARKETPLACE INFRASTRUCTURE VERIFICATION FAILED: $Message"
}
...
function Address-Word([string]$Address) {
    if ($Address -notmatch '^0x[0-9a-fA-F]{40}$') { Fail-R10Verify "Invalid constructor address." }
...
function Uint-Word([object]$Value) {
    ...
    if ($number.Sign -lt 0) { Fail-R10Verify "Negative constructor uint." }
    $hex = $number.ToString("x")
    if ($hex.Length -gt 64) { Fail-R10Verify "Constructor uint too large." }
...
function Invoke-RelicVerify(
    ...
    Fail-R10Verify "Unable to verify $ContractId after 3 attempts."
}

Import-RelicDotEnv (Join-Path $RepoPath ".env")
if ([string]::IsNullOrWhiteSpace($env:ETHERSCAN_API_KEY)) { Fail-R10Verify "ETHERSCAN_API_KEY missing." }
if (-not (Test-Path -LiteralPath $ManifestPath)) { Fail-R10Verify "Manifest missing." }

Set-Location -LiteralPath $RepoPath
$m = Get-Content -LiteralPath $ManifestPath -Raw | ConvertFrom-Json

Write-Host "Etherscan Free-tier mode: conservative sequential verification" -ForegroundColor Yellow
Write-Host "Pacing: >= 2.2 seconds between completed verifications" -ForegroundColor Yellow
...
Write-Host "R10 marketplace shared infrastructure source verification: COMPLETE" -ForegroundColor Green
